...
export function deriveDebtBand(dueMinutes: number, budget: number, config = getDailySchedulerConfig()): DebtBand {
...
function allocateMinutes(budget: number, debtBand: DebtBand, config: DailySchedulerConfig): DailyPlan["allocations"] {
  ...
  const result = Object.fromEntries(raw.map(({ key, exact }) => [key, Math.floor(exact)])) as DailyPlan["allocations"];
...
function newNodeLimit(budget: number, debtBand: DebtBand, config: DailySchedulerConfig): number {
  if (debtBand === "severe") return config.new_primary_node_limits.severe_debt;
  const preset = budget <= 20 ? config.new_primary_node_limits["20"] : budget <= 35 ? config.new_primary_node_limits["35"] : config.new_primary_node_limits["50"];
  return debtBand === "high" ? Math.min(preset, config.new_primary_node_limits.high_debt_max) : preset;
}

export function buildDailyPlan(input: DailySchedulerInput, config = getDailySchedulerConfig()): DailyPlan {
  ...
  const rankedReviews = input.reviewCandidates
    ...
    .sort((a, b) => a.priority - b.priority || Date.parse(a.candidate.dueAt ?? input.now) - Date.parse(b.candidate.dueAt ?? input.now) || a.candidate.id.localeCompare(b.candidate.id));

  const dueMinutes = rankedReviews.reduce((sum, entry) => sum + Math.max(1, entry.candidate.estimatedMinutes), 0);
  ...
  const lapsedNodes = input.nodeCandidates.filter((node) => node.masteryStatus === "lapsed").map((node) => node.nodeId).sort();
  const continuationNodes = input.nodeCandidates.filter((node) => node.masteryStatus === "developing" && node.reviewedContentAvailable).map((node) => node.nodeId).sort();
  const newCandidates = input.nodeCandidates
    .filter((node) => node.masteryStatus === "unseen" && node.eligibleForNewLearning && node.reviewedContentAvailable)
  ...
  const applicationNodes = input.nodeCandidates
    .filter((node) => node.applicationCapable && (selectedNewNodes.includes(node.nodeId) || continuationNodes.includes(node.nodeId) || lapsedNodes.includes(node.nodeId)))
  ...
  if ((debtBand === "high" || debtBand === "severe") && newCandidates.length > selectedNewNodes.length) deferredReasonCodes.push("review_debt");
  if (newCandidates.length > selectedNewNodes.length && !deferredReasonCodes.includes("review_debt")) deferredReasonCodes.push("time_budget");

  const lessonRequests: DailyPlan["lesson_requests"] = [];
  const coreNodes = [...new Set([...lapsedNodes, ...continuationNodes, ...selectedNewNodes])];
  const coreMinutes = Math.max(2, allocations.remediation + allocations.new_learning + Math.min(allocations.review, usedReviewMinutes));
  if (coreNodes.length > 0 || selectedReviews.length > 0) {
    lessonRequests.push({
      block_type: "mixed_core",
      minutes: coreMinutes,
      blueprint_type: selectedNewNodes.length > 0 ? "new_learning" : selectedReviews.length > 0 ? "review" : "mixed_learning",
  ...
  if (selectedNewNodes.length > 0) explanations.push(`Selected ${selectedNewNodes.length} reviewed curriculum frontier node(s) whose hard prerequisites are ready.`);
  if (deferredDue > 0) explanations.push(`${deferredDue} due review unit(s) were deferred rather than extending today's session.`);
  if (debtBand === "severe") explanations.push("New learning is paused because review debt exceeds the severe threshold.");
