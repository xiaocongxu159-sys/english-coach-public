...
At the same time, the existing manifest exposed its only icon as `purpose: "maskable"`, a weaker compatibility choice for older iOS Home Screen manifest-icon handling than an icon whose purpose includes `any`.
